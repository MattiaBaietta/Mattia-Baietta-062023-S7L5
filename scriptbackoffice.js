let ulpos = document.getElementsByClassName("rowBackoffice")[0]
fetch("https://striveschool-api.herokuapp.com/api/product/", {
    ...
    .then(data => data.forEach(element => {
        addBackoffice(element)
    }))


function addBackoffice(data) {
    console.log(data)
    ulpos.innerHTML += `<div class="col-3">
    <div class="card">
        ...
        <div class="d-flex justify-content-center ">
            <button class="rimuovi d-none" onclick='delCheck("${data._id}")'>Elimina</button>
            <button class="rimuovi d-none" onclick='openModal("${data.name}","${data.brand}","${data.description}","${data.imageUrl}","${data.price}","${data._id}")'>Modifica</button>
